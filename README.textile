...
Contentment has a number of dependencies that are not published packages (yet):

<pre><code>git clone http://github.com/marrow/marrow.util.git
(cd marrow.util; python setup.py develop)
 
git clone http://github.com/marrow/marrow.script.git
(cd marrow.script; python setup.py develop)
 
git clone http://github.com/marrow/marrow.blueprint.git
(cd marrow.blueprint; python setup.py develop)</code></pre>

Then clone and install Contentment.
...
Blueprints have been added to Contentment allowing you to bootstrap new sites and production configurations using an interactive question-and-answer system.  To use this, ensure the `marrow.blueprint`, `marrow.script`, and `marrow.util` dependencies are installed then run (at the top level of your virtual environment):

<pre><code>$ blueprint create site .</code></pre>

You will then be prompted for things like the name of the site, administrative user name, e-mail address, and password, and the database connection string.  Upon completion the blueprint will have created a number of stub folders (mostly in var) for Beaker data, bootstrapped the database with a default site and administrative user, and created a `development.ini` file for you.

h4. Creating the Initial Site (Old)

Then run the following to create a basic site in a MongoDB database called @cms@:
...
To use a site powered entirely by Contentment and Contentment modules (components, themes, extensions, etc.) all you need is an INI file.  If bootstrapping using Blueprint, a development.ini will have been created for you in the top level of your virtual environment.  If not, one is available within the Contentment folder.
