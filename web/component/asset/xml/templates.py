...
: def export_document record, recursive=False, level=0, root=False
: from web.component.asset.xml import get_simple_fields, get_complex_fields
\
	: if root
...
${INDENT * (level + 1)}${_bless(field)}\
	...
	: if recursive
		: for child in getattr(record, 'children', [])
			: yield from export_document(child, True, level+1)
		: end
	: end
${INDENT * level}</${name}>
	: if root
...
: def block record, level
	: for entry in iterate(export_document(record, level=1))
...
: from web.component.asset.xml import process_field
: if not field
	: return
: end
<${name}>
...
: def export_embedded_document record, name, field
